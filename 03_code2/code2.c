...
{
  int  starCount=0;
  int i;  //start with starCount being 0
  int j;
  for(i=1;i<=size;i++) //count from 0 (inclusive) to size (exclusive), for each number i that you count
    {
      for(j=1;j<=i;j++)   //count from 0 (inclusive) to i (inclusive), for each number j that you count
      {      
       printf("*");      //print a "*"
